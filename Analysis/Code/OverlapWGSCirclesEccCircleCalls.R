...
palmtrees_onlymatched = palmtrees %>% filter(Sample %in% merged_wgscircles_ecc$Sample)
...
samples = unique(merged_wgscircles_ecc$Sample)
for (i in 1:length(samples)){
  merged_wgscircles_ecc %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/forUCSCSession/PalmTreeAnalysis_WGSCirclesEcc_", samples[i], ".bed"),
...
for (i in 1:length(samples)){
  merged_wgscircles_ecc %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/WGSCircleBeds/PalmTreeAnalysis_WGSCirclesEcc_", samples[i], ".bed"),
                quote=F, sep="\t", row.names=F, col.names=F) 
}
...
all_random_regions = mclapply(1:nrow(palmtrees_onlymatched), function(i){
  ...
  circles_within_this_palmtree = merged_wgscircles_ecc %>% 
    filter(Sample ==  palmtrees_onlymatched[[i, "Sample"]],
           ...
           hasOverlap(palmtrees_onlymatched[[i, "FirstElement"]],
                      ...
                      merged_wgscircles_ecc$CircleStart,
                      merged_wgscircles_ecc$CircleEnd)) %>%
...
all_random_regions$CircleCalling = "WGS_eccDNA"

save.image("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEcc/WGSCircleEccOverlap_RandomSampling.Rdata")
...
for (svi in 1:length(svcallers)){
  ...
  ggplot(data=MeanDistr, aes(x = MeanOverlap)) + 
    ...
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEcc/WGSCircleEccOverlap_", svcallers[svi], "_PermutationTest.pdf"), height=4, width=6)
  ...
  ggplot(data=distr, aes(x=Overlap, color=isPerm)) + 
    ...
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEcc/WGSCircleEccOverlap_", svcallers[svi], "_PalmTreesVsRandomIntervals.pdf"), height=4, width=6)
}
