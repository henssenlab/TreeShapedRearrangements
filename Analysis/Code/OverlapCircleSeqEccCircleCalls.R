...
palmtrees_onlymatched = palmtrees %>% filter(Sample %in% cscircles_ecc$Sample)
...
samples = unique(cscircles_ecc$Sample)
for (i in 1:length(samples)){
  cscircles_ecc %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/forUCSCSession/PalmTreeAnalysis_CSCirclesEcc_", samples[i], ".bed"),
              quote=F, sep="\t", row.names=F, col.names=F) 
}

## Compute overlap of each palm trees with eccDNA and generate matching random regions 
samples = unique(cscircles_ecc$Sample)
palmtrees_onlymatched = palmtrees %>% filter(Sample %in% cscircles_ecc$Sample)
...
for (i in 1:length(samples)){
  cscircles_ecc %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/CSCircleBeds/PalmTreeAnalysis_CSCirclesEcc_", samples[i], ".bed"),
                quote=F, sep="\t", row.names=F, col.names=F) 
}
...
all_random_regions = mclapply(1:nrow(palmtrees_onlymatched), function(i){
  ...
  circles_within_this_palmtree = cscircles_ecc %>% 
    filter(Sample ==  palmtrees_onlymatched[[i, "Sample"]],
           ...
           hasOverlap(palmtrees_onlymatched[[i, "FirstElement"]],
                      ...
                      cscircles_ecc$CircleStart,
                      cscircles_ecc$CircleEnd)) %>%
...
all_random_regions$CircleCalling = "CircleSeq_eccDNA"

save.image("~/Desktop/PalmTrees/Results/Figures/RelationToCSCirclesEcc/CSCircleEccOverlap_RandomSampling.Rdata")
...
for (svi in 1:length(svcallers)){
  ...
  ggplot(data=MeanDistr, aes(x = MeanOverlap)) + 
    ...
    ggtitle(paste0("Test for eccDNA within Palm Tree Regions (CS calls; ", svcallers[svi], ")")) +
    ylab("Density") +
    ...
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToCSCirclesEcc/CSCircleEccOverlap_", svcallers[svi], "_PermutationTest.pdf"), height=4, width=6)
  ...
  ggplot(data=distr, aes(x=Overlap, color=isPerm)) + 
    ...
    theme(text = element_text(family = "Helvetica"),
          ...
          axis.title =  element_text(size = 10), 
          plot.title = element_text(face="plain", size = 12, hjust=0.5)) + 
    ggtitle(paste0("Overlap of Palm Trees with eccDNA (CS calls; ", svcallers[svi], ")")) +
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToCSCirclesEcc/CSCircleEccOverlap_", svcallers[svi], "_PalmTreesVsRandomIntervals.pdf"), height=4, width=6)
}
