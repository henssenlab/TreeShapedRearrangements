...
palmtrees_onlymatched = palmtrees 
txptinfo_onlymatched = txptinfo 
matching_samples = unique(palmtrees_onlymatched$Sample)

# for the UCSC browser
samples = unique(merged_wgscircles_ec$Sample)
for (i in 1:length(samples)){
  merged_wgscircles_ec %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/forUCSCSession/PalmTreeAnalysis_WGSCirclesEc_", samples[i], ".bed"),
              quote=F, sep="\t", row.names=F, col.names=F) 
}

## Compute overlap of each palm trees with ecDNA and generate matching random regions 
samples = unique(palmtrees$Sample) #unique(merged_wgscircles_ec$Sample)
palmtrees_onlymatched = palmtrees #%>% filter(Sample %in% merged_wgscircles_ec$Sample)
...
for (i in 1:length(samples)){
  merged_wgscircles_ec %>%
    filter(Sample == samples[i], CircleChr != "chrM") %>%
    dplyr::select(CircleChr, CircleStart, CircleEnd) %>%
    write.table(paste0("~/Desktop/PalmTrees/Results/Tables/WGSCircleBeds/PalmTreeAnalysis_WGSCirclesEc_", samples[i], ".bed"),
                quote=F, sep="\t", row.names=F, col.names=F) 
}
...
all_random_regions = mclapply(1:nrow(palmtrees_onlymatched), function(i){
  ...
  circles_within_this_palmtree = merged_wgscircles_ec %>% 
    filter(Sample ==  palmtrees_onlymatched[[i, "Sample"]],
           ...
           hasOverlap(palmtrees_onlymatched[[i, "FirstElement"]],
                      ...
                      merged_wgscircles_ec$CircleStart,
                      merged_wgscircles_ec$CircleEnd)) %>%
...
all_random_regions$CircleCalling = "WGS_ecDNA"

save.image("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEc/WGSCircleEcOverlap_RandomSampling.Rdata")
...
for (svi in 1:length(svcallers)){
  ...
  ggplot(data=MeanDistr, aes(x = MeanOverlap)) + 
    ...
    ggtitle(paste0("Test for ecDNA within Palm Tree Regions (WGS calls; ", svcallers[svi], ")")) +
    ylab("Density") +
    ...
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEc/WGSCircleEcOverlap_", svcallers[svi], "_PermutationTest.pdf"), height=4, width=6)
  ...
  ggplot(data=distr, aes(x=Overlap, color=isPerm)) + 
    ...
    theme(text = element_text(family = "Helvetica"),
          ...
          axis.title =  element_text(size = 10), 
          plot.title = element_text(face="plain", size = 12, hjust=0.5)) + 
    ggtitle(paste0("Overlap of Palm Trees with ecDNA (WGS calls; ", svcallers[svi], ")")) +
    ggsave(paste0("~/Desktop/PalmTrees/Results/Figures/RelationToWGSCirclesEc/WGSCircleEcOverlap_", svcallers[svi], "_PalmTreesVsRandomIntervals.pdf"), height=4, width=6)
}
